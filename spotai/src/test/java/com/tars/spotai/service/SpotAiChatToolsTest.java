...
class SpotAiChatToolsTest {
    ...
    @Test
    void recommendShopsFiltersByMaxPriceAndSortsByScore() {
    ...
    void recommendShopsReturnsEmptyWhenNoMatch() {
        ...
        assertThat(tools.recommendShops(30L, "")).isEqualTo("[]");
    }

    @Test
    void searchShopReturnsShopsByKeyword() {
        ShopService shopService = mock(ShopService.class);
    ...
    @Test
    void queryShopDetailReturnsShopInfo() {
        ShopService shopService = mock(ShopService.class);
    ...
    void queryReviewSummaryReturnsSummaryData() {
        ...
        summary.setHighlights(List.of("服务好", "环境棒"));
    ...
    private static Voucher makeVoucher(Long id, String title, Long payValue, Long actualValue) {
        ...
        v.setId(id); v.setTitle(title); v.setPayValue(payValue); v.setActualValue(actualValue);
        return v;
    }

    private static Shop makeShop(Long id, String name, Long avgPrice, Integer score, String area) {
        Shop s = new Shop();
        s.setId(id); s.setName(name); s.setAvgPrice(avgPrice); s.setScore(score); s.setArea(area);
        return s;
    }
}